...
public class ParticipantRestController {
	...
	public ResponseEntity<?> getMeeting(@PathVariable("id") String login) {
	 ...
	 if (participantService.findByLogin(participant.getLogin()) != null) {
	 			 return new ResponseEntity("participant with login " + participant.getLogin() + " already exist.", HttpStatus.CONFLICT);
	 		 }
	 //dodac
	 ...
	 @RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
	 	 public ResponseEntity<?> deleteParticipant(@PathVariable("id") String login){
	 		 if (participantService.findByLogin(login) == null) {
	 			 return new ResponseEntity("participant with login " + login + " do not exist.", HttpStatus.CONFLICT);
	 		 }
	 		 Participant participant = participantService.findByLogin(login);
	 		 participantService.deleteParticipant(participant);
	 		 return new ResponseEntity("participant with login " + login + " deleted.", HttpStatus.OK);
	 }
}
